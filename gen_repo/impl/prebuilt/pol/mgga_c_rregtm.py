t1 = 3 ** (0.1e1 / 0.3e1)
t3 = (0.1e1 / math.pi) ** (0.1e1 / 0.3e1)
t5 = 4 ** (0.1e1 / 0.3e1)
t6 = t5 ** 2
t7 = r0 + r1
t8 = t7 ** (0.1e1 / 0.3e1)
t11 = t1 * t3 * t6 / t8
t14 = math.sqrt(t11)
t17 = t11 ** 0.15e1
t19 = t1 ** 2
t20 = t3 ** 2
t22 = t8 ** 2
t25 = t19 * t20 * t5 / t22
t31 = math.log(0.1e1 + 0.16081979498692535067e2 / (0.37978500000000000000e1 * t14 + 0.89690000000000000000e0 * t11 + 0.20477500000000000000e0 * t17 + 0.12323500000000000000e0 * t25))
t33 = 0.621814e-1 * (0.1e1 + 0.53425000000000000000e-1 * t11) * t31
t34 = r0 - r1
t35 = t34 ** 2
t36 = t35 ** 2
t37 = t7 ** 2
t38 = t37 ** 2
t42 = t34 / t7
t43 = 0.1e1 + t42
t44 = t43 <= p_a_zeta_threshold
t45 = p_a_zeta_threshold ** (0.1e1 / 0.3e1)
t46 = t45 * p_a_zeta_threshold
t47 = t43 ** (0.1e1 / 0.3e1)
t49 = lax_cond(t44, t46, t47 * t43)
t50 = 0.1e1 - t42
t51 = t50 <= p_a_zeta_threshold
t52 = t50 ** (0.1e1 / 0.3e1)
t54 = lax_cond(t51, t46, t52 * t50)
t55 = t49 + t54 - 0.2e1
t56 = 2 ** (0.1e1 / 0.3e1)
t57 = t56 - 0.1e1
t59 = 0.1e1 / t57 / 0.2e1
t60 = t55 * t59
t71 = math.log(0.1e1 + 0.32163958997385070134e2 / (0.70594500000000000000e1 * t14 + 0.15494250000000000000e1 * t11 + 0.42077500000000000000e0 * t17 + 0.15629250000000000000e0 * t25))
t84 = math.log(0.1e1 + 0.29608749977793437516e2 / (0.51785000000000000000e1 * t14 + 0.90577500000000000000e0 * t11 + 0.11003250000000000000e0 * t17 + 0.12417750000000000000e0 * t25))
t85 = (0.1e1 + 0.27812500000000000000e-1 * t11) * t84
t89 = t36 / t38 * t60 * (-0.3109070e-1 * (0.1e1 + 0.51370000000000000000e-1 * t11) * t71 + t33 - 0.19751673498613801407e-1 * t85)
t91 = 0.19751673498613801407e-1 * t60 * t85
t92 = math.log(0.2e1)
t93 = 0.1e1 - t92
t94 = math.pi ** 2
t97 = t45 ** 2
t98 = t47 ** 2
t99 = lax_cond(t44, t97, t98)
t100 = t52 ** 2
t101 = lax_cond(t51, t97, t100)
t103 = t99 / 0.2e1 + t101 / 0.2e1
t104 = t103 ** 2
t105 = t104 * t103
t112 = 0.1e1 / t93
t119 = math.exp(-(-t33 + t89 + t91) * t112 * t94 / t105)
t120 = t119 - 0.1e1
t124 = s0 + 0.2e1 * s1 + s2
t139 = (0.1e1 + 0.27801896084645508333e-2 * (0.1e1 + 0.25000000000000000000e-1 * t11) / (0.1e1 + 0.44450000000000000000e-1 * t11) * t112 * t94 / t120 * t124 / t8 / t37 * t56 / t104 * t19 / t3 * t5) ** (0.1e1 / 0.4e1)
t145 = math.log(0.1e1 + 0.10000000000000000000e1 * (0.1e1 - 0.1e1 / t139) * t120)
t147 = t93 / t94 * t105 * t145
t148 = r0 ** (0.1e1 / 0.3e1)
t149 = t148 ** 2
t153 = t43 / 0.2e1
t154 = t153 ** (0.1e1 / 0.3e1)
t155 = t154 ** 2
t156 = t155 * t153
t158 = r1 ** (0.1e1 / 0.3e1)
t159 = t158 ** 2
t163 = t50 / 0.2e1
t164 = t163 ** (0.1e1 / 0.3e1)
t165 = t164 ** 2
t166 = t165 * t163
t169 = 0.1e1 / t22 / t37
t173 = 6 ** (0.1e1 / 0.3e1)
t175 = t94 ** (0.1e1 / 0.3e1)
t176 = t175 ** 2
t177 = 0.1e1 / t176
t182 = 0.5e1 / 0.9e1 * (tau0 / t149 / r0 * t156 + tau1 / t159 / r1 * t166 - t124 * t169 / 0.8e1) * t173 * t177 / (t156 + t166)
t184 = math.log(DBL_EPSILON)
t187 = t184 / (-t184 + 0.64e0)
t190 = lax_cond(t182 < -t187, t182, -t187)
t195 = math.exp(-0.64e0 * t190 / (0.1e1 - t190))
t196 = lax_cond(-t187 < t182, 0, t195)
t198 = math.log(0.14285714285714285714e1 * DBL_EPSILON)
t201 = (-t198 + 0.15e1) / t198
t202 = t182 < -t201
t203 = lax_cond(t202, -t201, t182)
t207 = math.exp(0.15e1 / (0.1e1 - t203))
t209 = lax_cond(t202, 0, -0.7e0 * t207)
t210 = lax_cond(t182 <= 0.1e1, t196, t209)
t214 = 0.1e1 / (0.1e1 + 0.44450000000000000000e-1 * t14 + 0.31385250000000000000e-1 * t11)
t217 = math.exp(0.10000000000000000000e1 * t214)
t220 = t56 ** 2
t226 = (0.1e1 + 0.21337642104376358333e-1 * t173 * t177 * t220 * t124 * t169) ** (0.1e1 / 0.4e1)
t231 = math.log(0.1e1 + (t217 - 0.1e1) * (0.1e1 - 0.1e1 / t226))
t239 = t36 ** 2
t241 = t38 ** 2
res = -t33 + t89 + t91 + t147 + t210 * ((-0.285764e-1 * t214 + 0.285764e-1 * t231) * (0.1e1 - 0.2363e1 * t57 * t55 * t59) * (0.1e1 - t239 * t36 / t241 / t38) + t33 - t89 - t91 - t147)
